...
import { orders, Order, refetchOrders } from '../store/orders';
import { User } from '../store/users';

const OrderTableRow: Component<{ order: Order; index: () => number }> = ({ order, index }) => {
	return (
		<tr>
			<td>{index() + 1}</td>
			<td>
		...
		<>
			...
			<table class='table table-dark'>
				...
				<tbody>
					<For
						each={
							user == null ? orders() : orders().filter((order) => order.customer_id == user().id)
						}
					>
